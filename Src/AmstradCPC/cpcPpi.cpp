...
namespace CPC {
    ...
    void CPpi::ResetVars()
    {
        m_portDirections[PORT_A] = DIRECTION_OUTPUT;
        m_portDirections[PORT_B] = DIRECTION_INPUT;
        m_portDirections[PORT_C_UPPER] = DIRECTION_OUTPUT;
        m_portDirections[PORT_C_LOWER] = DIRECTION_OUTPUT;

        m_portOutputValue[PORT_A] = 0;
        m_portOutputValue[PORT_B] = 0;
        m_portOutputValue[PORT_C_UPPER] = 0;
        m_portOutputValue[PORT_C_LOWER] = 0;
    }
    ...
    cpcByte CPpi::ReadPortA()
    {
        ...
        KMASSERTM(m_portDirections[PORT_A] == DIRECTION_INPUT, ("Trying to read from PPI port A when it is currently configured as OUTPUT."));
        if (m_portDirections[PORT_A] == DIRECTION_INPUT)
        {
    ...
    {
        KMASSERTM(m_portDirections[PORT_A] == DIRECTION_OUTPUT, ("Trying to write to PPI port A when it is currently configured as INPUT."));
        if (m_portDirections[PORT_A] == DIRECTION_OUTPUT)
        {
            // Save the value into the internal register
            m_portOutputValue[PORT_A] = nValue;
    ...
    {
        KMASSERTM(m_portDirections[PORT_B] == DIRECTION_OUTPUT, ("Trying to write to PPI port B when it is currently configured as INPUT."));
        if (m_portDirections[PORT_B] == DIRECTION_OUTPUT)
        {
            // Save the value into the internal register
            m_portOutputValue[PORT_B] = nValue;
        }
    }
    ...
    cpcByte CPpi::ReadPortC()
    {
        ...
        if ((m_portDirections[PORT_C_UPPER] == DIRECTION_OUTPUT) && (m_portDirections[PORT_C_LOWER] == DIRECTION_OUTPUT))
        {
            // The port is configured as output, so when a read operation gives the value of the port internal output register
            nRet = m_portOutputValue[PORT_C];
        }
    ...
    void CPpi::WritePortC(cpcByte nValue)
    {
        ...
        if (m_portDirections[PORT_C_UPPER] == DIRECTION_OUTPUT)
        {
            ...
            m_portOutputValue[PORT_C] = (nValue & 0xF0) | (m_portOutputValue[PORT_C] & 0x0F);
        ...
        if (m_portDirections[PORT_C_LOWER] == DIRECTION_OUTPUT)
        {
            ...
            m_portOutputValue[PORT_C] = (m_portOutputValue[PORT_C] & 0xF0) | (nValue & 0x0F);
    ...
    void CPpi::WriteControlWord(cpcByte nValue)
    {
        ...
        if (nValue & 0x80)      // If bit 7 is "1"...
        {
            ...
            m_portDirections[PORT_B] = eDirection;
        ...
        else
        {
            ...
            if (nValue & 0x01)
            {
                m_portOutputValue[PORT_C] = m_portOutputValue[PORT_C] | (1 << nBit);    // Set the bit
            }
            else
            {
